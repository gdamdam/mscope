 ...
 * guard the vendoring — that the module graph resolves under mscope's tsconfig
...
describe('vendored mbus-client', () => {
  ...
  it('builds outbound frames that parse back as JSON', () => {
    ...
    expect(JSON.parse(outbound.announce('mscope'))).toEqual({
      type: 'mbus/announce',
      name: 'mscope',
    })
  })
  ...
  it('parses a welcome frame and ignores non-mbus traffic', () => {
    const welcome = JSON.stringify({
      ...
      clientId: 'c1',
      mbus: MBUS_VERSION,
      sources: [{ sourceId: 's1', name: 'mscope', clientId: 'c2' }],
    })
